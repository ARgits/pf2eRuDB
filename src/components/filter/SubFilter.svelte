<script lang="ts">
  ...
  import StatusIcon from "@components/utilityComponents/statusIcon.svelte";
  ...
  function changeOptionState(key: keyof filterUnion, val: string) {
    ...
    console.log($globalFilters[$dataKey][key].defaultValue);
  ...
  $: filterNotDefault = () => {
    if ($globalFilters[$dataKey][key].disabled.length) return true;
    if ($globalFilters[$dataKey][key].value.length !== $globalFilters[$dataKey][key].defaultValue.length) return true;
    return !$globalFilters[$dataKey][key].value.every((val) =>
      $globalFilters[$dataKey][key].defaultValue.includes(val),
    );
  };
</script>
...
<div class="filter_item">
  ...
  <div class="filter_label" on:click={changeVisibility}>
    <span class="label">{$globalFilters[$dataKey][key].name}</span>
    {#if filterNotDefault()}
      <ResetButton {key} />
    {/if}
    <span class="caret">
